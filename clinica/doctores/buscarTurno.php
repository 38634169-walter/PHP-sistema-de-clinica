            ...
            <?php
                if( (isset($_POST['dni'])) or (isset($_POST['fecha'])) ){
                    ...
                    if( $_POST['dni'] and $_POST['fecha'] ){
                        $sql="SELECT * FROM turnos WHERE dniPaciente = '".$_POST['dni']."' AND fecha = '".$_POST['fecha']."' AND doctor = '".$_SESSION['ID_usuario']."' ";
                    }
                    else{
                        if($_POST['dni']){
                            $sql="SELECT * FROM turnos WHERE dniPaciente = '".$_POST['dni']."' AND doctor = '".$_SESSION['ID_usuario']."' ";
                        }
                        if($_POST['fecha']){
                            $sql="SELECT * FROM turnos WHERE fecha = '".$_POST['fecha']."' AND doctor = '".$_SESSION['ID_usuario']."' ";
                        }
                    }
                        ...
                        echo "
                            <div class='cartel-aviso' style='display:flex;justify-content:center;align-items:center;position:relative;z-index:100;'>
                                <div style='width:90%;background:red;border-radius:15px;padding:10px;display:flex;flex-direction:column;justify-content:center;align-items:center;position:fixed;top:3cm;z-index:9999999;'>
                                    ...
                                    <a href='turnos.php' class='btn-cartel-aviso' style='text-decoration=none;padding:0px 5px;text-align:center;color:black;background:white;border-radius:10px;'>Ok</a>
                                </div>    
                    </div>    
                        ";  
                    }
